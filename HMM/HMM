...
def genertate_initial_vector(hidden_states):
    ...
    count = 0 # 计算总数
    with open("./data/nt.txt",'r',encoding='utf-8') as nrfile:
        all_data = nrfile.readlines()
        all_data.remove(all_data[54])
    ...
    with open("./data/initial_vector.txt","w",encoding='utf-8') as outputfile:
...
def generate_transition_probability(hidden_states):
    ...
    result = []
    with open("./data/nt.tr.txt","r",encoding='utf-8') as initial_count_file:
    ...
    #输出、写入文件
    with open("./data/transition_probability.txt","w",encoding='utf-8') as output_file:
    ...
    with open("./data/nt.txt","r",encoding='utf-8') as nrfile:
        ...
        for line in all_data:
            ...
            for index in range(0,len(tags_and_freq),2):
                ...
                result.append([tmp_list[0],observed_state,float(tmp_list[1])/initial_freq[tmp_list[0]]])#tmp_list

    # 输出、写入文件
    with open("./data/emit_probability.txt","w",encoding='utf-8') as output_file:
        for thelist in result:
